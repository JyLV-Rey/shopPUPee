    ...
    <div class="max-w-6xl mx-auto px-4 py-10">
        ...
        <div class="flex items-center gap-3 mb-8">
            ...
            @if ($cartItems->isNotEmpty())
                <span class="badge badge-primary" id="item-count-badge">{{ $cartItems->count() }}
        ...
        @if ($cartItems->isEmpty())
            <div class="flex flex-col items-center justify-center py-32 gap-4 text-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-20 w-20 text-base-content/20" fill="none"
        ...
        @else
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    @foreach ($grouped as $sellerName => $items)
                        <div class="rounded-xl border border-base-200 bg-base-100 shadow-sm overflow-hidden">
                            ...
                            <div class="flex items-center gap-2 px-4 py-3 bg-base-200/50 border-b border-base-200">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 text-primary shrink-0"
                                    viewBox="0 0 20 20" fill="currentColor">
                                    <path
                                        d="M3 1a1 1 0 000 2h1.22l.305 1.222a.997.997 0 00.01.042l1.358 5.43-.893.892C3.74 11.846 4.632 14 6.414 14H15a1 1 0 000-2H6.414l1-1H14a1 1 0 00.894-.553l3-6A1 1 0 0017 3H6.28l-.31-1.243A1 1 0 005 1H3zM16 16.5a1.5 1.5 0 11-3 0 1.5 1.5 0 013 0zM6.5 18a1.5 1.5 0 100-3 1.5 1.5 0 000 3z" />
                                </svg>
                                <span
                                    class="text-xs font-semibold text-base-content/70 uppercase tracking-wide">{{ $sellerName }}</span>
                            </div>

                            @foreach ($items as $item)
                                @php $product = $item->product; @endphp
                                <div class="flex items-center gap-4 px-4 py-4 {{ !$loop->last ? 'border-b border-base-200' : '' }}"
                                    data-cart-item="{{ $item->cart_item_id }}" data-price="{{ $product->price }}"
                                    data-qty="{{ $item->quantity }}">

                                    {{-- Checkbox --}}
                                    <label class="flex items-center cursor-pointer">
                                        <input type="checkbox" class="checkbox checkbox-primary checkbox-sm cart-select"
                                            value="{{ $item->cart_item_id }}" checked />
                                    </label>
                                    ...
                                    <a href="{{ route('product.view', $product->product_id) }}" class="shrink-0">
                                        @if ($product->images->isNotEmpty())
                                            <img src="{{ $product->images->first()->image_url }}"
                                                alt="{{ $product->name }}"
                                                class="w-16 h-16 rounded-lg object-cover border border-base-200"
                                                style="image-rendering: pixelated;" />
                                        @else
                                            <div
                                                class="w-16 h-16 rounded-lg bg-base-200 flex items-center justify-center shrink-0">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-base-content/20"
                                    ...
                                    <div class="flex-grow min-w-0">
                                        <a href="{{ route('product.view', $product->product_id) }}"
                                            class="text-sm font-semibold hover:text-primary line-clamp-1 leading-snug">{{ $product->name }}</a>
                                    ...
                                    <div class="flex flex-col items-end gap-2 shrink-0">
                                        <span
                                            class="text-sm font-bold text-primary subtotal">₱{{ number_format($product->price * $item->quantity, 2) }}</span>
                                        <form method="POST" action="{{ route('cart.destroy', $item->cart_item_id) }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-xs btn-ghost text-error px-1"
                                                title="Remove">
                ...
                <div class="lg:col-span-1">
                    <div
                        class="rounded-xl border border-base-200 bg-base-100 shadow-sm p-5 sticky top-24 flex flex-col gap-4">
                        <div class="flex items-center justify-between">
                            <h2 class="font-bold text-center text-base">Order Summary</h2>
                            <label class="ml-5 flex items-center gap-1.5 text-xs text-base-content/50 cursor-pointer">
                                <input type="checkbox" id="select-all" class="checkbox checkbox-primary checkbox-xs"
                                    checked />
                                Select All
                            </label>
                        </div>
                        ...
                        <a href="{{ route('search') }}" class="btn btn-ghost btn-block btn-sm text-xs">Continue
                            Shopping</a>
                    </div>
                </div>
