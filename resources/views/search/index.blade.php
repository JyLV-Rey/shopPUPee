...
@section('content')
    ...
    <div class="max-w-7xl mx-auto px-4 py-6">
        ...
        <p class="text-lg text-base-content/60 mb-6">
            Showing results for "<span class="font-medium">{{ $searchTerm }}</span>"
            @if ($searchCategory)
                in category "<span class="font-medium">{{ $searchCategory }}</span>"
            @endif
            @if ($searchStore)
                from store "<span class="font-medium">{{ $searchStore }}</span>"
            @endif
        </p>
        {{-- Search Controls --}}
            ...
            <div class="card bg-base-100 shadow-sm border border-base-200">
                <div class="card-body p-4 sm:p-6">
                        ...
                        <div class="form-control flex-1">
                            <div class="join w-full">
                                <input type="text" name="searchTerm" value="{{ $searchTerm }}"
                                    placeholder="Search products..." class="input input-bordered join-item w-full" />
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
                        ...
                        {{-- Store --}}
                        <div class="form-control">
                            <input type="text" name="searchStore" value="{{ $searchStore }}" placeholder="Store name..."
                    ...
                    <div class="flex flex-wrap items-center justify-between gap-3 mt-2">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="isDescending" value="1"
                                class="checkbox checkbox-primary checkbox-sm" @checked($isDescending) />
